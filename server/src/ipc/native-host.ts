...
export class NativeHostConnection {
  ...
  private findHostPath(): string {
    ...
    throw new Error(
      'Native host not found. Please install the Chrome extension first.\n' +
  ...
  private processMessages(): void {
    while (this.messageBuffer.length >= 4) {
      ...
      try {
        const message = JSON.parse(msgStr) as NativeMessage;
...
let defaultConnection: NativeHostConnection | null = null;

/**
 * Get the default native host connection (creates one if needed)
 */
export function getDefaultConnection(): NativeHostConnection {
