    ...
    if (elapsed > REVALIDATION_INTERVAL_MS) {
      ...
      if (!result.valid && elapsed > OFFLINE_GRACE_PERIOD_MS) {
        return {
          ...
          remaining: 0,
          message:
            "Pro license could not be validated (offline for 7+ days). Please check your internet connection and try again, or set RETHINKSOFT_IN_CHROME_LICENSE_KEY in your environment.",
        };
      }
    }
